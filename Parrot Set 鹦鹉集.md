...
(base) PS E:\Project\Parrot-Set> netstat -ano | findstr :8000
  TCP    0.0.0.0:8000           0.0.0.0:0              LISTENING       26248
(base) PS E:\Project\Parrot-Set> taskkill /PID 26248 /F
成功: 已终止 PID 为 26248 的进程。
...
1. 再将多个 units 合并到目标长度（chunk_size=500 字符）：

- 尽量保持语义完整（不截断段落）

- 如果单个 unit 超过 500 字符，则对该 unit 使用固定长度切片

1. 重叠策略（overlap=80 字符）：

- 前一块末尾的 80 个字符会作为下一块的前缀，保留上下文
